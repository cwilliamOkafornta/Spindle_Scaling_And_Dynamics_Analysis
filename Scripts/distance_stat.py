...
######################################################################################################################################
'''
The following functions are used to process the csv files generated after  extracting the positions of the centrosomes and 
chromosomes  from the kymographs. The positions maybe generated from MatLab customs code or python code. This script is tested 
for the csv files generated from MatLab code.
...
The next step is to exctract similar columns in each file and add them to a created csv file with a given name. The new files are 
...
def distanceCalculation(files):
    ...
    for file in files:
        ...
        file_save = df.to_csv(os.path.join(r'', file), index=False)
