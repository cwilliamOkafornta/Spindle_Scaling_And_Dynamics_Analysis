...
# calculate for cell length from the cell volume
def cellVolumeLength(cell_volume_file_number, parameter_files):
    cell_volume = parameter_files[cell_volume_file_number]
...
# calculate for the mean and standard deviation of the chromosome-to-chromosome distance for each embryonic cell stage
def chromosomeDistance(chromosome_file_number, parameter_files):
    ...
    for col_chromosome in chromosome_to_chromosome_distance:
        ...
        chromosome_mean = pd.concat([chromosome_mean, chrom_temp_val_mean], sort=False, axis=0)
        chromosome_std = pd.concat([chromosome_std, chrom_temp_val_std], sort=False, axis=0)
        
    # new table
    chromosome_distance = pd.concat([chromosome_mean, chromosome_std], sort=False, axis=1)
    return chromosome_distance

# calculate for the mean and standard deviation of the pole-to-pole distance for each embryonic cell stage
def poleDistance(poles_file_number, parameter_files):
    pole_to_pole_distance = parameter_files[poles_file_number]
    ...
    for col_pole in pole_to_pole_distance:
        ...
        poles_mean = pd.concat([poles_mean, pole_temp_val_mean], sort=False, axis=0)
        poles_std = pd.concat([poles_std, pole_temp_val_std], sort=False, axis=0)
        
    # new table 
    pole_distance = pd.concat([poles_mean, poles_std], sort=False, axis=1)
    return pole_distance

# calculate for the mean and standard deviation of the pole-to-chromosome distance for each embryonic cell stage
def poleChromosomeDistance(pole_chromosome_file_number, parameter_files):
    pole_to_chromosome_distance = parameter_files[pole_chromosome_file_number]
    ...
    for col_pole_chrom in pole_to_chromosome_distance:
        ...
        pole_chrom_mean = pd.concat([pole_chrom_mean, pole_chrom_temp_mean], sort=False, axis=0)
        pole_chrom_std = pd.concat([pole_chrom_std, pole_chrom_temp_std], sort=False, axis=0)
        
    # new tables
    polesChroms_distance = pd.concat([pole_chrom_mean, pole_chrom_std], sort=False, axis=1)
    return polesChroms_distance

# calculate for the mean and standard deviation of the metaphase pole-to-pole distance for each embryonic cell stage
def metaphasePoleDistance(metaphase_pole_file_number, parameter_files):
    metaphase_pole_to_pole_distance = parameter_files[metaphase_pole_file_number]
    ...
    for col_metaphase in metaphase_pole_to_pole_distance:
        ...
        metaphase_pole_mean = pd.concat([metaphase_pole_mean, meta_pole_temp_mean], sort=False, axis=0)
        metaphase_pole_std = pd.concat([metaphase_pole_std, meta_pole_temp_std], sort=False, axis=0)
        
    # new tables
    metaphase_distance = pd.concat([metaphase_pole_mean, metaphase_pole_std], sort=False, axis=1)
    return metaphase_distance

# calculate for the mean and standard deviation of the pole-to-pole elongation rate for each embryonic cell stage
def elongationRatePoles(elongation_speed_file_number, parameter_files):
    elongation_speed_pole_to_pole = parameter_files[elongation_speed_file_number]
    ...
    for col_elongation in elongation_speed_pole_to_pole:
        ...
        elongation_speed_mean = pd.concat([elongation_speed_mean, elong_speed_temp_mean], sort=False, axis=0)
        elongation_speed_std = pd.concat([elongation_speed_std, elong_speed_temp_std], sort=False, axis=0)
        
    # new tables
    elongation_speed = pd.concat([elongation_speed_mean, elongation_speed_std], sort=False, axis=1)
    return elongation_speed

# calculate for the mean and standard deviation of the chromosome-to-chromosome segregation speed for each embryonic cell stage
def segregationSpeedChromosomes(segregation_speed_file_number, parameter_files):
    segregation_speed_chromosome_to_chromosome = parameter_files[segregation_speed_file_number]
    ...
    for col_segregation in segregation_speed_chromosome_to_chromosome:
        ...
        segregation_speed_mean = pd.concat([segregation_speed_mean, elong_speed_temp_mean], sort=False, axis=0)
        segregation_speed_std = pd.concat([segregation_speed_std, elong_speed_temp_std], sort=False, axis=0)
        
    # new tables
    segregation_speed = pd.concat([segregation_speed_mean, segregation_speed_std], sort=False, axis=1)
    return segregation_speed

# calculate for the mean and standard deviation of the initial pole-to-pole distance for each embryonic cell stage
def initialPoleDistance(initial_pole_file_number, parameter_files):
    initial_pole_distance = parameter_files[initial_pole_file_number]
    ...
    for col_initial in initial_pole_distance:
        ...
        initial_mean = pd.concat([initial_mean, initial_temp_val_mean], sort=False, axis=0)
        initial_std = pd.concat([initial_std, initial_temp_val_std], sort=False, axis=0)
        
    # new tables
    initial_poles = pd.concat([initial_mean, initial_std], sort=False, axis=1)
    return initial_poles

# make a table with spindle dynamics values and save the table as a csv file in the save folder
def spindleDynamicsValues(cell_volume_file_number,
                          chromosome_file_number,
                          poles_file_number,
                          pole_chromosome_file_number,
                          metaphase_pole_file_number,
    ...
    final_pole_chromosome = poleChromosomeDistance(pole_chromosome_file_number, parameter_files)
    metaphase_length = metaphasePoleDistance(metaphase_pole_file_number, parameter_files)
    elongation = elongationRatePoles(elongation_speed_file_number, parameter_files)
    segregation = segregationSpeedChromosomes(segregation_speed_file_number, parameter_files)
    initial_pole_length = initialPoleDistance(initial_pole_file_number, parameter_files)
    
    # table with spindle dynamics values
    newTable = pd.concat([initial_pole_length, final_poles_length, final_chromosome_length, final_pole_chromosome, metaphase_length, elongation, segregation], sort=False, axis=1).reset_index(drop=True)
    new_df = pd.concat([cell_volume_length, newTable], sort=False, axis=1)
    
    # save table as csv file
    csv_new_df = new_df.to_csv(os.path.join(save_folder, save_file_name + '(' + embryo_condition + ')' + '.csv'), index=False, encoding='cp1252')
